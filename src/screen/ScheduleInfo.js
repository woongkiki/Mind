...
import { ScrollView, Platform, Dimensions, StyleSheet, Alert, View, TouchableOpacity, SafeAreaView, ActivityIndicator } from 'react-native';
...
const ScheduleInfo = (props) => {
    ...
    const scheduleAdd = () => {
        Api.send('schedule_update', {'sName':scheduleName, 'startDate':startDate, 'time':time, 'zip':addrText, 'addr1':addr1, 'addr2':addr2, 'cname':selectClient, 'cidx':selectClientIdx, 'important':important, 'alarm':alarm, 'categorys':categorys, 'mb_id':userInfo.mb_name, 'mb_idx':userInfo.mb_no, 'idx':params.idx}, (args)=>{
            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
        ...
        <Box flex={1} backgroundColor='#fff'>
            <HeaderDef headerTitle='스케줄 수정' navigation={navigation} />
            {
                ...
                <ScrollView>
                    <Box p='20px'>
                        <Box>
                            <HStack alignItems={'center'} mb='10px' justifyContent={'space-between'}>
                                <DefText text='일정 카테고리' style={[styles.labelTitle, {marginBottom:0}]} />
                                <TouchableOpacity onPress={()=>setDelModal(true)} style={{paddingHorizontal:10, height:24, backgroundColor:'#f00' ,justifyContent:'center', alignItems:'center', borderRadius:5}}>
            ...
            <Modal isOpen={delModal} onClose={()=>setDelModal(false)}>
                <Modal.Content p={0} borderRadius={0}>
                    <Modal.Body p={0}>
                        <Box py='20px' style={{justifyContent:'center', alignItems:'center'}}>
                            <DefText text='스케줄 삭제시 복구가 불가능합니다.' />
                            <DefText text={'현재 스케줄을 삭제하시겠습니까?.'} style={{marginTop:10}} />
                        </Box>
                        <HStack>
                            <SubmitButtons 
                                btnText={'예'}
                            ...
                            <SubmitButtons 
                                btnText={'아니오'}
                                buttonStyle={{width:(width-40) * 0.5, height:45, backgroundColor:colorSelect.gray}}
