...
const WMCunsult = (props) => {
    ...
    const renderApi = async () => {
        ...
        await Api.send('wm_consultCate', {}, (args)=>{
            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
                console.log('wm 컨설팅 카테고리 결과: ', arrItems, resultItem); 
                setWmBestCateList(arrItems);
            }else{
                console.log('wm 컨설팅 카테고리 실패!', resultItem);

            }
        });
        await Api.send('wm_consultList', {"cate":wmBestCate, "schText":searchText}, (args)=>{
            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
                console.log('wm 컨설팅 자료 결과: ', arrItems, resultItem); 
                setItemList(arrItems);
            }else{
                console.log('wm 컨설팅 자료 실패!', resultItem);
        ...
        return(
            <Box px='20px' mb='20px'>
                <Box borderWidth={1} borderColor='#999' borderRadius={10}>
                    <TouchableOpacity style={{padding:20}} onPress={()=>navigation.navigate("WMCunsultView", {"idx":item.wr_id})}>
                        <DefText 
