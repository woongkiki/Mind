...
const EducationDataView = props => {
  ...
  const edDataViewAPI = async () => {
    await setLoading(true);
    Api.send(
      'education_dataView',
      {wr_id: params.wr_id, id: userInfo?.mb_id},
      args => {
        let resultItem = args.resultItem;
        let arrItems = args.arrItems;

        if (resultItem.result === 'Y' && arrItems) {
          console.log('교육자료 상세결과: ', arrItems.file);
          setDataInfo(arrItems);
          setInfoFile(arrItems.file);

          if (arrItems.depth3 == '') {
            setDepthName(arrItems.depth2);
          } else if (arrItems.depth2 == '') {
            setDepthName(arrItems.depth1);
          } else {
            setDepthName(arrItems.depth3);
          }
        } else {
          console.log('교육자료 상세 실패!', resultItem);
        }
      },
    );
      ...
      {loading ? (
        ...
        <ScrollView>
          <Box p="20px">
            <Box>
              <DefText
                text={'[' + depthName + '] ' + dataInfo.wr_subject}
              ...
              <HStack justifyContent={'space-between'}>
                ...
                <HStack alignItems={'center'}>
                  ...
                  <DefText
                    text={
                      dataInfo != ''
                        ? dataInfo.wr_datetime.substring(0, 10)
                        : '-'
                    }
                  />
                </HStack>
                    ...
                    infoFile.map((item, index) => {
                      return (
                        <Box key={index} mt={index != 0 ? '10px' : 0}>
                          <TouchableOpacity
                            //onPress={()=>checkPermission(item.urls, item.bf_source)}
                            onPress={() =>
            ...
            <Box style={[styles.contentBox]}>
              {dataInfo != '' && <WebRender html={dataInfo.wr_content} />}
            </Box>
