      ...
      {analLoading ? (
        ...
        <ScrollView>
          <Box p="20px">
            <DefText
              text="나의 능력"
              style={[{fontSize: fsize.fs20}, fweight.b]}
            />
          </Box>
          <Box height={width} width={width} px="20px">
            <WebView
              source={{
                uri:
                  'https://maumapp.cafe24.com/chart2.php?idx=' + userInfo.mb_no,
              }}
            />
            <Box
              height={width}
          ...
          <Box height={width} width={width} backgroundColor="#00f">
            <WebView
              source={{
                uri:
                  'https://maumapp.cafe24.com/chart.php?idx=' + userInfo.mb_no,
              }}
            />
            <Box
              height={width}
          ...
          <Box p="20px" mt="-40px" pb="25px">
                ...
                {keyArr.map((item, index) => {
                  return (
                    <Box
                      key={index}
                      width="50%"
                      mb={keyArr.length == index + 1 ? 0 : '10px'}>
            ...
            <DefText
              text="분석 결과"
              style={[{fontSize: fsize.fs20, marginTop: 30}, fweight.b]}
            />
            <Box mt="15px">
              <DefText
                text={contents != '' ? contents : '등록된 분석내용이 없습니다.'}
                style={[{fontSize: fsize.fs16, lineHeight: 23}]}
              />
            </Box>
          </Box>
        </ScrollView>
