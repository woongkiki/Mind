...
const PointUseList = (props) => {
    ...
    const handlerSubmit = async () => {
       ...
       await Api.send('price_point', {'idx':userInfo.mb_no}, (args)=>{
        ...
        await Api.send('price_pointUseList', {'idx':userInfo.mb_no, 'mb_id':userInfo.mb_id,'date':category}, (args)=>{
            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
            ...
            <Box px='20px' mb={'20px' }>
                <Box key={index} p='15px' backgroundColor={'#fff'} borderRadius={10} shadow={9}>
                    <HStack mb='10px'>
                        ...
                        <Box>
                            <DefText text={item.po_datetime} />
                        </Box>
                    </HStack>
                
                    <HStack mb='10px'>
                        <Box style={[styles.pointLeft]}>
                            <DefText text='사용 포인트' style={[styles.pointLabel]} />
                        </Box>
                        <Box>
                            <DefText text={ '-' + numberFormat(item.po_use_point) + 'P'} style={[{color:colorSelect.blue}]}/>
                        </Box>
                    </HStack>
                    <HStack>
                        <Box style={[styles.pointLeft]}>
                        ...
                        <Box>
                            <DefText text={numberFormat(item.po_mb_point) + 'P'} style={[{color:colorSelect.orange}]} />
                        </Box>
                    </HStack> 
                </Box>
            </Box>
    ...
    return (
        <Box flex={1} backgroundColor='#fff'> 
            <HeaderDef headerTitle='포인트 사용내역' navigation={navigation} />
            {
                !pointLoading ?
                <FlatList 
                    ListHeaderComponent={
                        <Box p='20px'>
                            <Box>
                                ...
                                <HStack mt='15px' alignItems={'center'} justifyContent='space-between'>
                                    ...
                                    <TouchableOpacity style={{padding:5, backgroundColor:colorSelect.blue, paddingHorizontal:10, borderRadius:5}}>
