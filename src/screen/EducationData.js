...
const EducationData = props => {
  ...
  const dataHandler = async () => {
    ...
    await Api.send(
      'com_dataCategory',
      {category: category, schText: searchText},
      args => {
    ...
    await Api.send(
      ...
      args => {
        ...
        if (resultItem.result === 'Y' && arrItems) {
          console.log('동영상 교육자료 내역 결과: ', resultItem);
          setDataBoard(arrItems);
        } else {
  ...
  const _renderItem = ({item, index}) => {
    ...
    return (
      <Box px="20px" key={index} mb="15px">
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('EducationView', {idx: item.wr_id})
          }>
              ...
              <Box width="80%">
                <DefText
                  text={textLengthOverCut(
                    '[' + item.wr_1 + '] ' + item.wr_subject,
                    20,
                  )}
                  style={[styles.videoTitle]}
                />
                <DefText
                  text={item.wr_datetime.substring(0, 10)}
                  style={[{color: colorSelect.gray, marginTop: 10}]}
                />
              </Box>
              {item.file_name != '' && (
                <TouchableOpacity
                  onPress={() => checkPermission(fileDown, item.file_name)}>
                  <Image
